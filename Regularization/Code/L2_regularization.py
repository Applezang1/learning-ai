...
# Define 30 input/output pairs to use as training data
...
# Define Plotting Function for Gabor Model
...
# Define a function to compute the Gabor Model's loss
...
# Define a function to compute the derivative of the loss function with respect to phi0
...
# Define a function to compute the derivative of the loss function with respect to phi1
...
# Define a function to compute the gradient using the previously defined derivative functions
def compute_gradient(data_x, data_y, phi):
    dl_dphi0 = gabor_deriv_phi0(data_x, data_y, phi[0],phi[1])
    dl_dphi1 = gabor_deriv_phi1(data_x, data_y, phi[0],phi[1])
    return np.array([[dl_dphi0],[dl_dphi1]])

# Define a Gradient Descent Step Function
...
# Compute the Loss of the Gabor Model with Defined Parameters
loss =  compute_loss(data[0,:], data[1,:], model, phi_all[:,0:1])

# Plot the Loss Function
draw_model(data,model,phi_all[:,0:1], "Initial parameters, Loss = %f"%(loss))

# Compute the Gradient Descent Step Over Itterations
...
# Define Plotting Function to plot the regularization function
...
# Define Plotting Function for Loss Function with Regularization
...
# Plot the Loss Function with Regularization
draw_loss_function_reg(data, model, 0.2, my_colormap)

# Define a Function to Compute the Derivative of the Modified Loss Function with Respect to Phi0   
def dregdphi0(phi, lambda_):
  deriv = 2*lambda_*phi

  return deriv

# Define a Function to Compute the Derivative of the Modified Loss Function with Respect to Phi1
def dregdphi1(phi, lambda_):
  deriv = 2*lambda_*(phi - 12.5)

  return deriv

# Define a Function to Compute the Gradient of the Modified Loss Function Using the Defined Functions Above
...
# Define a function to compute the gradient descent step
...
# Compute the loss of the inital neural network model
loss =  compute_loss2(data[0,:], data[1,:], model, phi_all[:,0:1], lambda_)

# Plot the initial Gabor Model
draw_model(data,model,phi_all[:,0:1], "Initial parameters, Loss = %f"%(loss))

# Compute the gradient descent step with regularization
for c_step in range (n_steps):
