...
# Define 30 input/output pairs of training data
...
# Define a function to plot the Gabor model
...
# Define a function to plot the least squares loss function
...
# Define a function to compute the derivative of the Gabor model in respect to phi0
...
# Define a function to compute the derivative of the Gabor model in respect to phi1
...
# Define a function to compute the gradient of the Gabor model by using the defined derivatives
...
# Compute the loss of the initial Gabor model
...
# Compute the loss of the final Gabor model
loss =  compute_loss(data[0,:], data[1,:], model, phi_all[:,c_step+1:c_step+2])

# Plot the final Gabor model
draw_model(data,model,phi_all[:,c_step+1], "Iteration %d, loss = %f"%(c_step+1,loss))

# Plot the trajectory of the loss function over itterations
...
# Compute the initial loss of the initial Gabor model
loss =  compute_loss(data[0,:], data[1,:], model, phi_all[:,0:1])

# Plot the initial Gabor model
draw_model(data,model,phi_all[:,0:1], "Initial parameters, Loss = %f"%(loss))

# Repeatedly take stochastic gradient descent steps (with momentum)
for c_step in range (n_steps):
  # Choose random batch indices
  batch_index = np.random.permutation(data.shape[1])[0:batch_size]
  # Compute the gradient
  gradient = compute_gradient(data[0,batch_index], data[1,batch_index],  phi_all[:,c_step:c_step+1])
  # Compute the momentum and incorporate it into the stochastic gradient descent step
...
# Repeatedly take stochastic gradient descent steps (with Nesterov momentum)
for c_step in range (n_steps):
  # Choose random batch indices
  batch_index = np.random.permutation(data.shape[1])[0:batch_size]
  # Compute the Nesterov momentum and incorporate it into the stochastic gradient descent step
...
# Plot the trajectory of the loss function over iterations
draw_loss_function(compute_loss, data, model,phi_all)
